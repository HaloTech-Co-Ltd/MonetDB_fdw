...
make_new_connection(ConnCacheEntry *entry, UserMapping *user)
{
	...
	elog(DEBUG3, "new monetdb_fdw connection %p for server \"%s\" (user mapping oid %u, userid %u)",
...
connect_pg_server(ForeignServer *server, UserMapping *user)
{
	...
	PG_TRY();
	{
		...
		/* Use "monetdb_fdw" as fallback_application_name */
		keywords[n] = "fallback_application_name";
		values[n] = "monetdb_fdw";
		n++;
		...
		if (entry->xact_depth > 0)
		{
			...
			switch (event)
			{
				...
				case XACT_EVENT_PRE_COMMIT:
					 ...
					 * constrains how old a server monetdb_fdw can
